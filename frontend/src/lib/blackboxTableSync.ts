import type { CourierTableRow, DualCommissionCourierRow } from "../utils/courierTableParse";

export const DEFAULT_BLACKBOX_CHAT_ID = "dI1nCuH";

const LS_MIRROR_KEY = (chatId: string) => `blackbox:table-mirror:${chatId}`;
const LS_MIRROR_DUAL_KEY = (chatId: string) => `blackbox:table-mirror:dual:${chatId}`;

/** Union of row shapes stored in Blackbox mirror / sync payloads. */
export type BlackboxCourierMirrorRow = CourierTableRow | DualCommissionCourierRow;
...
export function readBlackboxMirrorDual(chatId: string): DualCommissionCourierRow[] | null {
  try {
    const raw = localStorage.getItem(LS_MIRROR_DUAL_KEY(chatId));
    if (!raw) return null;
    const data = JSON.parse(raw) as unknown;
    if (!Array.isArray(data)) return null;
    return data as DualCommissionCourierRow[];
  } catch {
    return null;
  }
}

export function writeBlackboxMirrorDual(chatId: string, rows: DualCommissionCourierRow[]): void {
  try {
    localStorage.setItem(LS_MIRROR_DUAL_KEY(chatId), JSON.stringify(rows));
  } catch {
    /* quota / private mode */
  }
}
...
/** Same endpoint as legacy sync; rows use `DualCommissionCourierRow` shape. */
export async function syncDualCommissionCourierRowsToBlackbox(
  chatId: string,
  rows: DualCommissionCourierRow[],
): Promise<{ ok: boolean; error?: string }> {
  const endpoint = import.meta.env.VITE_BLACKBOX_TABLE_SYNC_URL as string | undefined;
  writeBlackboxMirrorDual(chatId, rows);
