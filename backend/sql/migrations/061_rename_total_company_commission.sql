-- Rename settlement aggregate: total_company_commission = SUM(driver_payouts.company_commission).
BEGIN;
...
CREATE OR REPLACE FUNCTION subcontractor_settlement_totals(
    ...
    total_company_commission NUMERIC(12, 2),
...
COMMENT ON FUNCTION subcontractor_settlement_totals(UUID, DATE, DATE) IS
    'Per-subcontractor SUM of driver_payouts for the period. total_company_commission = SUM(company_commission); total_payable = SUM(net_driver_payout). subcontractor_payouts stores payment status only.';
