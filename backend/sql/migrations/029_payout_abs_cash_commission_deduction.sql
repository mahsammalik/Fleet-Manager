-- Glovo / Excel parity: transfer commission stays signed (negative TVT → negative transfer_commission).
-- Cash commission is stored signed, but driver payout subtracts ABS(cash_commission) so the fleet
-- percentage applies to daily cash volume when "Plata zilnica cu cash" is negative.
