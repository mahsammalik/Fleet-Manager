...
try {
  const sql = fs.readFileSync(path.join(migrationsDir, "064_assignment_vehicle_rent.sql"), "utf8");
  console.log("Applying 064_assignment_vehicle_rent.sql...");
  await pool.query(sql);
  console.log("  OK");

  const verifySql = fs.readFileSync(
    path.join(migrationsDir, "064_assignment_vehicle_rent.verify.sql"),
    "utf8",
  );
  console.log("Running 064 verify...");
